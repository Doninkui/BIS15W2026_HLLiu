...
ggmap(map)+
  geom_point(data=sharks,
             ...
             color="darkblue",
             alpha=0.7)+
  labs(title = "shark attacks observations in california",
       x="longtitude",
       y="latitude")
```
...
```
```{r}
lat_fa <- c(32.855,39.578)
lon_fa <- c(-117.28,-123.7783)
bbox_fa <- make_bbox(lon_fa,lat_fa,f=0.05)
```
```{r}
map_fa <- 
  get_stadiamap(bbox_fa,
...
lon_sd <- c(-118.1,-117.1)
bbox_sd <- make_bbox(lon_sd,lat_sd,f=0.03)
```
```{r}
map_sd <- 
  get_stadiamap(bbox_sd,maptype = "stamen_terrain",zoom=9)
```
```{r}
ggmap(map_sd)+
  geom_point(data = sharks_sd,
             aes(longitude,latitude),
             size=0.6,
             color="darkblue",
             alpha=0.8)
